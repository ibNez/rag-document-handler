...
if [ "$DEV_MODE" = true ]; then
    ...
    echo "   2. Start application: ${BLUE}python app.py${NC}"
...
echo "   ${BLUE}./status.sh${NC}        # Check project status"
echo "   ${BLUE}./setup.sh --help${NC}  # Show setup options"
echo "   ${BLUE}./uninstall.sh${NC}     # Complete removal"
echo ""
echo "🏗️  Container Management:"
echo "   ${BLUE}docker compose up -d${NC}              # Start all services"
echo "   ${BLUE}docker compose up postgres milvus -d${NC} # Database services only"
echo "   ${BLUE}docker compose down${NC}                # Stop all services"
echo "   ${BLUE}docker compose logs -f webui${NC}       # View webui logs"
